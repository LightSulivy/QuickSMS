...
def calculate_selling_price(api_price):
    if api_price is None: return None
...
async def on_ready():
    ...
    conn = sqlite3.connect('database.db')
    cursor = conn.cursor()
    cursor.execute("SELECT order_id, discord_id, price, service, status FROM orders WHERE status='PENDING'")
    ...
    for order_id, user_id, price, service_name, status in pending_orders:
        try:
            ...
            if user:
                ...
                asyncio.create_task(check_sms_loop(order_id, dm_channel, view, dm_message=None))
                count += 1
        except Exception as e:
    ...
    print(f"✅ {count} commandes en attente reprises. Bot connecté en tant que {bot.user}")
...
async def deposit(interaction: discord.Interaction, amount: float, user: discord.Member):
    ...
    await interaction.followup.send(f"✅ Compte de {user.mention} crédité de {amount}€. Nouveau solde : {get_balance(user.id):.2f}€", ephemeral=True)
...
async def services(interaction: discord.Interaction, country: app_commands.Choice[str] = None):
    ...
    for i, res in enumerate(results):
        ...
        if res is not None:
            ...
            emoji = "📱"
            if "whatsapp" in svc_name.lower(): emoji = "🟢"
            elif "telegram" in svc_name.lower(): emoji = "🔵"
...
async def execute_pack_logic(interaction: discord.Interaction):
    ...
    price1_brut, price2_brut = await asyncio.gather(
        sms_api.get_price(svc1_code, ctry1_id),
        sms_api.get_price(svc2_code, ctry2_id)
    )

    if price1_brut is None or price2_brut is None:
         return await interaction.followup.send("⚠️ Un des services du pack est indisponible pour le moment.", ephemeral=True)
    ...
    try:
        ...
        embed = discord.Embed(title="📦 Confirmation Pack", description="Détail du Pack **Whatsapp (FR) + Telegram (CA)**", color=0xe91e63)
        embed.add_field(name="1. Whatsapp (France)", value=f"{price1:.2f}€", inline=True)
        embed.add_field(name="2. Telegram (Canada)", value=f"{price2:.2f}€", inline=True)
        embed.add_field(name="💰 PRIX TOTAL", value=f"**{total_price:.2f}€**", inline=False)
        embed.set_footer(text="Le débit se fera étape par étape.")
        
        view = ConfirmPackView(interaction.user.id, price1, steps)
        await dm_channel.send(embed=embed, view=view)
        await interaction.followup.send("📩 Confirmation envoyée en MP. Vérifiez vos messages !", ephemeral=True)
        
    except discord.Forbidden:
        await interaction.followup.send("❌ Impossible de vous envoyer un MP. Ouvrez vos messages privés.", ephemeral=True)
    except Exception as e:
...
class ConfirmPackView(discord.ui.View):
    ...
    async def confirm(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self.clear_items()
        await interaction.edit_original_response(content="✅ **Lancement du pack...**", view=self)
        ...
        await execute_buy_logic(interaction, "whatsapp", "france", next_steps=self.steps)
...
async def execute_buy_logic(interaction: discord.Interaction, service_key: str, country_key: str, next_steps=None):
    user_id = interaction.user.id
    
    if service_key not in SERVICES or country_key not in COUNTRIES:
        return await interaction.followup.send("❌ Service ou pays invalide (Configuration).", ephemeral=True)
    ...
    if cost_price is None:
        return await interaction.followup.send(f"⚠️ Stock épuisé ou erreur prix pour **{service_name}** ({country_name}). Réessayez plus tard.", ephemeral=True)
    ...
    if balance < price:
        return await interaction.followup.send(f"❌ **Solde insuffisant.**\nPrix : {price:.2f}€\nVotre solde : {balance:.2f}€", ephemeral=True)
    ...
    if not order or not order['success']:
        ...
        return await interaction.followup.send(f"❌ Impossible d'obtenir un numéro pour {service_name}. \nRaison : {error_msg}", ephemeral=True)
    ...
    conn.execute("INSERT INTO orders (order_id, discord_id, phone, price, status, created_at, service, cost) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", 
                 (order['id'], user_id, order['phone'], price, "PENDING", str(datetime.now()), service_name, cost_price))
    conn.commit()
    conn.close()
    ...
    try:
        dm_channel = await interaction.user.create_dm()
        view = OrderView(order['id'], price, user_id, interaction, next_steps=next_steps)
        ...
        await interaction.followup.send(f"✅ Numéro acheté ! Regardez vos MP.", ephemeral=True)
        
        # Lancement du check
        asyncio.create_task(check_sms_loop(order['id'], dm_channel, view, dm_message))
        
    except Exception as e:
        await interaction.followup.send(f"❌ Erreur lors de l'envoi du MP (Ouvrez vos MP !) : {e}", ephemeral=True)
...
class OrderView(discord.ui.View):
    def __init__(self, order_id, price, user_id, original_interaction, next_steps=None):
    ...
    async def finish(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.next_steps:
             ...
             await interaction.followup.send(f"🔄 **Lancement automatique de la suite du pack : {next_svc.capitalize()} ({next_ctry.capitalize()})...**", ephemeral=True)
             ...
             await execute_buy_logic(interaction, next_svc, next_ctry, next_steps=remaining_steps)
...
async def setup_dashboard(guild):
    ...
    async for message in channel.history(limit=5):
        if message.author == bot.user and message.embeds and message.embeds[0].title == "🚀 QuickSMS Dashboard":
            last_message = message
            break
            
    view = DashboardView()
    embed = discord.Embed(title="🚀 QuickSMS Dashboard", description="Bienvenue ! Utilisez les boutons ci-dessous pour commander.", color=0x3498db)
...
class DashboardView(discord.ui.View):
    ...
    @discord.ui.button(label="🛒 Acheter un numéro", style=discord.ButtonStyle.primary, custom_id="dash_buy")
    async def buy_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message("🌍 **Étape 1 : Choisissez le pays**", view=CountrySelectView(), ephemeral=True)

    @discord.ui.button(label="📱 Services & Prix", style=discord.ButtonStyle.secondary, custom_id="dash_services")
    async def services_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("🌍 **Voir les prix pour quel pays ?**", view=CountrySelectView(mode="prices"), ephemeral=True)

    @discord.ui.button(label="💰 Mon Solde", style=discord.ButtonStyle.success, custom_id="dash_balance")
    async def balance_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        balance = get_balance(interaction.user.id)
        await interaction.response.send_message(f"💰 **Votre solde : {balance:.2f}€**", ephemeral=True)

    @discord.ui.button(label="📦 Pack: WA(FR) + TG(CA)", style=discord.ButtonStyle.danger, custom_id="dash_pack_wa_tg_v2")
...
class CountrySelect(discord.ui.Select):
    def __init__(self, options, mode):
        self.mode = mode
        super().__init__(placeholder="Sélectionnez un pays...", min_values=1, max_values=1, options=options)

    async def callback(self, interaction: discord.Interaction):
        country_key = self.values[0]
        
        if self.mode == "buy":
            await interaction.response.send_message(f"📱 **Pays : {country_key.capitalize()}**. Choisissez le service :", view=ServiceSelectView(country_key), ephemeral=True)
        else:
            # Mode Prix
            await interaction.response.defer(ephemeral=True)
            ...
            embed = discord.Embed(title=f"📱 Services & Prix ({country_key.capitalize()})", color=0x00ff00)
            description = ""
...
class ServiceSelect(discord.ui.Select):
    def __init__(self, options, country_key):
        ...
        super().__init__(placeholder="Sélectionnez un service...", min_values=1, max_values=1, options=options)
